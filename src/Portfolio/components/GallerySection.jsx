...
const GallerySection = () => {
  ...
  const galleryImages = [
    ...
    { id: 5, src: "./Oracle AI Vector Search Certified Professional.png", title: "Oracle AI Vector Search", category: "certification" },
    { id: 6, src: "./Oracle Cloud Infrastructure 2025 Certified Generative AI Professional.png", title: "Oracle Cloud Infrastructure", category: "certification" },
    { id: 7, src: "./Oracle Fusion AI Agent Studio Certified Foundations Associate - Rel 1.png", title: "Oracle Fusion AI Agent", category: "certification" },
    { id: 8, src: "./Build Generative AI Apps and Solutions with No-Code Tools.png", title: "Generative AI Apps", category: "course" },
    { id: 9, src: "./ChatGPT-4 Prompt Engineering ChatGPT Generative AI & LLM.png", title: "ChatGPT Prompt Engineering", category: "course" },
    { id: 10, src: "./Computational_Theory_Language_Principle_Finite_Automata_Theory.png", title: "Computational Theory", category: "course" },
    { id: 11, src: "./Fundamentals of Network Communication.png", title: "Network Communication", category: "course" },
    { id: 12, src: "./Introduction to Git and GitHub by Google.png", title: "Git & GitHub", category: "course" },
  ];
            ...
            return (
              ...
              >
                ...
                <div className="absolute inset-0 bg-black/50 opacity-0 hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                  <span className="text-white text-xs text-center p-1">{image.title}</span>
                </div>
              </div>
            );
          })}
  ...
  const GridView = () => (
    <div 
      ...
      onMouseLeave={() => setIsPaused(false)}
    >
      <div className="flex gap-4 pb-4" style={{ width: `${galleryImages.length * 320}px` }}>
        {galleryImages.map((image, index) => {
          const dims = imageDimensions[image.id] || { width: 300, height: 200, aspectRatio: 1.5 };
          const maxHeight = 200; // Base height
          const calculatedHeight = Math.max(maxHeight, dims.height > maxHeight ? maxHeight : dims.height);
          const containerHeight = calculatedHeight;
          
          return (
            <div
              key={image.id}
              className="flex-shrink-0 rounded-xl overflow-hidden cursor-pointer transition-all duration-500 hover:scale-105 hover:shadow-2xl group"
              style={{
            ...
            >
              <div className="relative w-full h-full">
                <img
                  ...
                  style={{ objectFit: dims.aspectRatio > 1.5 ? 'contain' : 'cover' }}
                  onError={(e) => {
                    e.target.src = "./title.png";
                  }}
                />
  ...
  const HybridView = () => {
    const featuredDims = imageDimensions[galleryImages[0]?.id] || { width: 600, height: 400, aspectRatio: 1.5 };
    const featuredHeight = Math.max(320, featuredDims.height > 320 ? 320 : featuredDims.height);
    
    return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {/* Featured large image */}
      <div className="md:col-span-2 lg:col-span-2 relative group cursor-pointer rounded-2xl overflow-hidden transition-all duration-500 hover:scale-102 hover:shadow-2xl"
        ...
        <div className="relative w-full h-full">
          <img
            ...
            className="w-full h-full object-contain transition-transform duration-700 group-hover:scale-110"
            style={{ objectFit: featuredDims.aspectRatio > 1.5 ? 'contain' : 'cover' }}
            onError={(e) => {
              e.target.src = "./title.png";
            }}
          />
          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500">
            <div className="absolute bottom-4 left-4 right-4">
              <h3 className="text-white font-bold text-lg mb-2">{galleryImages[0].title}</h3>
              <div className="flex items-center gap-2">
                <span className="bg-white/20 backdrop-blur-sm px-2 py-1 rounded text-white text-xs">Featured</span>
                <span className="text-white/70 text-sm capitalize">{galleryImages[0].category}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {galleryImages.slice(7).map((image, index) => {
        const dims = imageDimensions[image.id] || { width: 300, height: 200, aspectRatio: 1.5 };
        const gridHeight = Math.max(192, dims.height > 192 ? 192 : dims.height);
  ...
  return (
    <section id="gallery" className="py-20 px-4 sm:px-6 bg-gradient-to-br from-gray-900 via-purple-900/20 to-gray-900">
      <style jsx>{`
        @keyframes float {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-bold mb-6 bg-clip-text text-transparent bg-gradient-to-r from-purple-400 via-pink-400 to-purple-400">
            Gallery Showcase
          </h2>
          <div className="w-32 h-1 mx-auto rounded-full bg-gradient-to-r from-purple-400 to-pink-400"></div>
          <p className="mt-6 text-gray-300 max-w-2xl mx-auto">
            Explore my collection of certifications, achievements, and moments through an interactive gallery experience
          </p>
        </div>
