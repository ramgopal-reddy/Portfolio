...
export function ExperienceSection() {
  ...
  return (
    <section id="experience" className={`py-20 px-4 sm:px-6 transition-all duration-300 ${theme.isDark ? 'bg-gray-800/50' : 'bg-gray-100/50'}`}> {/* bg-gray-800/50 - dark background with transparency */}
          ...
          {experienceData.map((exp, index) => (
            ...
            >
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
                <h3 className="text-2xl font-semibold flex items-center gap-2" style={{ color: theme.colors.text }}>
                  <Briefcase className="w-5 h-5" style={{ color: theme.colors.primary }} />
                  {exp.title}
                </h3>
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {exp.skills.map((skill, index) => (
                  <span
                    key={index}
                    className="px-3 py-1 text-xs rounded-full border" style={{ backgroundColor: `${theme.colors.primary}20`, color: theme.colors.primary, borderColor: `${theme.colors.primary}30` }}
                  >
                    {skill}
                  </span>
                ))}
              ...
              {exp.outcome && (
                <div className="flex items-start gap-2 text-sm" style={{ color: theme.colors.textSecondary }}>
                  <Award className="w-4 h-4 mt-[2px]" style={{ color: theme.isDark ? '#10b981' : '#059669' }} />
